...
public class UserProfileServiceTest {
    ...
    private static String EMAIL_EXPEXTED = "linus@linux.com";
    private static String EMAIL_ACTUAL = "wiz@aa.aa";


    private User testUserExpected;
    private User testUserActual;
    private UserProfile testUserProfileExpected;
    private UserProfile testUserProfileActual;
    private UserProfileRepository moockedProfileRepo;

    @Before
    public void init() {
        this.moockedProfileRepo = Mockito.mock(UserProfileRepository.class);

        this.testUserExpected = new User() {{
            setUsername("linus.linux.com");
            setEmail(EMAIL_EXPEXTED);
        }};
    ...
    @Test
    public void userProfileService_ShouldReturnCorrectProfile_On_Given_Username() {

        Mockito.when(
            this.moockedProfileRepo.findByProfileOwner_Username(USERNAME_EXPECTED))
               .thenReturn(Optional.ofNullable(this.testUserProfileExpected));

        UserProfile expected = this.testUserProfileExpected;
        UserProfile actual = this.moockedProfileRepo.findByProfileOwner_Username("wiz.aa.aa").get();
